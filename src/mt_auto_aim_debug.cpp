...
// === 修改 / MODIFIED ===
// 包含了新的gimbal头文件，并注释掉了旧的cboard头文件。
...
int main(int argc, char * argv[])
{
  ...
  // === 修改 / MODIFIED ===
  // 使用新的 Gimbal 类来替代 CBoard 类进行初始化。
  ...
  // === 修改 / MODIFIED ===
  // 将 mode 的类型修改为 GimbalMode 以匹配 gimbal.mode() 的返回值。
  ...
  while (!exiter.exit()) {
    ...
    // === 修改 / MODIFIED ===
    // 1. 从 gimbal 对象获取云台状态(state)和模式(mode)。
    // 2. 使用 gimbal.q() 函数替代 cboard.imu_at() 来获取IMU姿态。
    // 3. 使用 gimbal.state() 获取弹速等状态信息。
    ...
    if (last_mode != mode) {
      // === 修改 / MODIFIED ===
      // 使用 gimbal.str(mode) 来获取模式的字符串表示，用于日志打印。
    ...
    auto targets = tracker.track(armors, t);
      ...
      // === 修改 / MODIFIED ===
    // 将从 gimbal.state() 中获取的弹速 gs.bullet_speed 传递给决策线程。
