  ...
  omni_target_priority_{ArmorPriority::fifth}
{
  ...
  use_ukf_ = yaml["use_ukf"].as<bool>(false);
...
bool Tracker::set_target(std::list<Armor> & armors, std::chrono::steady_clock::time_point t)
{
  ...
  if (is_balance) {
    ...
    target_ = Target(armor, t, 0.2, 2, P0_dig, use_ukf_);
  }

  else if (armor.name == ArmorName::outpost) {
    Eigen::VectorXd P0_dig{{1, 64, 1, 64, 1, 81, 0.4, 100, 1e-4, 1e-10, 1e-10}};
    target_ = Target(armor, t, 0.2765, 3, P0_dig, use_ukf_);
  }

  else if (armor.name == ArmorName::base) {
    Eigen::VectorXd P0_dig{{1, 64, 1, 64, 1, 64, 0.4, 100, 1e-4, 0, 0}};
    target_ = Target(armor, t, 0.3205, 3, P0_dig, use_ukf_);
  }

  else {
    Eigen::VectorXd P0_dig{{1, 64, 1, 64, 1, 64, 0.4, 100, 1, 1, 1}};
    target_ = Target(armor, t, 0.2, 4, P0_dig, use_ukf_);
  }
